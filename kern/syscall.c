...
sys_exofork(void)
{
    ...
    struct Env *e = NULL;
    int val;
    if ((val = env_alloc(&e, curenv->env_id)) < 0) {
        return val;
    }
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
    ...
    struct Env *e = NULL;
    // if environment envid doesnt exist or caller has no perms
    if ((envid2env(envid, &e, 1) < 0)) {
        return -E_BAD_ENV;
    }

    // if va >= UTOP or va is not page alligned
    if ((uintptr_t)va >= UTOP || ((uintptr_t)va % PGSIZE) != 0) {
        return E_INVAL;
    }

    // if theres no memory to allocate the new page
    if (!(p = page_alloc(0))) {
        return -E_NO_MEM;
    }

    // check perms and allocate page
    if ((page_insert(e->env_pgdir, p, va, PTE_U | PTE_P | perm)) < 0) {
        page_free(p);
        return -1;
    }
    return 0;
