...
mem_init(void)
{
    ...
    envs = (struct Env *)boot_alloc(NENV * sizeof(struct Env) + 1);
    ...
    boot_map_region(kern_pgdir, UPAGES, size, PADDR(pages), perm);
    //perm = PTE_W | PTE_P;
    //boot_map_region(kern_pgdir, UPAGES, size, P_ADDR(pages), perm);
	...
	// LAB 3: Your code here.
    perm = PTE_U;
    boot_map_region(kern_pgdir, UENVS, PTSIZE, PADDR(envs), perm);
...
page_init(void)
{
	...
	// free pages!
    // Physical page 0 in use
	size_t i;
    page_free_list = NULL;

    // Create linked list of free pages
	for (i = 0; i < npages; i++) {
        if (i == 0 || i*PGSIZE == MPENTRY_PADDR || (i*PGSIZE >= IOPHYSMEM  && i*PGSIZE < PADDR(boot_alloc(0)))) {
            pages[i].pp_ref = 1;
...
page_free(struct PageInfo *pp)
{
    ...
    if (pp->pp_ref != 0 || pp->pp_link != NULL) {
        panic("pp_ref is non zero or not null");
    }
    else {
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    // relevant page doesn't exist yet
    if ((*pde & PTE_P) == 0)  {
        if(create == 0) {
            return NULL;
        }
        ...
        // If allocationf fails
        if (!pp_page_table) { return NULL; }

        // Increment ref count
        pp_page_table->pp_ref += 1;

        // Put the page into the directory with permissions
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
   ...
   for (int i = 0; i < size/PGSIZE; ++i) {
        pte_t * p_pte = pgdir_walk(pgdir, (void *)(i*PGSIZE + va), 1);
        *p_pte = (PTE_ADDR(pa) + i*PGSIZE) | perm | PTE_P;
    }
}
...
//
// the pte for this page.  This is used by page_remove and
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    uint32_t i;
    pte_t *p;
    // go though the range of memory
    for (i = (uint32_t)ROUNDDOWN((uint32_t)va, PGSIZE); i < (uint32_t)ROUNDUP((uint32_t)va+len, PGSIZE); i += PGSIZE) {
        p = pgdir_walk(env->env_pgdir, (void *)i, 0);
        user_mem_check_addr = (uintptr_t)i;
        if (!p || i >= ULIM || (*p & PTE_P) == 0 || (*p & perm) != perm) {
            if (i < (uintptr_t)va) {
